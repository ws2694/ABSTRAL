...
**Step 2.4**: Token budget allocation - PREDICTOR agents must complete structured JSON output before adding verbose reasoning. Reserve 700 tokens minimum for required JSON fields (risk_score, label, confidence, model_agreement, reasoning, key_risk_factors). Truncate verbose clinical reasoning if approaching token limit but ensure all JSON fields are populated. <!-- [Evidence: trace P04560, iter 2] -->
...
**Step 6**: Low-risk optimization pattern - for cases with stable low OPS (<0.4), CCI ≤ 1, and no bone-depleting medications, use stream