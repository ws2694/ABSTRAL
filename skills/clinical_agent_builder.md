...
- **ML ensemble score fidelity**: When ML prediction succeeds, the ensemble score must be the primary determinant of risk assessment. Clinical reasoning may adjust within ±0.1 of ensemble score but cannot invert the prediction (low ML score → high prediction or vice versa). Document any adjustments with specific clinical justification. <!-- [Evidence: trace P00363, iter 1] -->
- **Mandatory OPS clinical override**: For stable high OPS (≥0.6 maintained >12 months), risk score must be ≥0.8 regardless of ML ensemble output. This represents critically hospitable bone environment requiring clinical judgment override of ML predictions. Document override reasoning explicitly. <!-- [Evidence: trace P06640, iter 1] -->
...
- **T1 (single agent) routing**: Use for cases with OPS ≤ 0.35, CCI ≤ 1, AND minimal medication events (≤2). These cases require only basic ML prediction with OPS interpretation. Pipeline topology adds unnecessary complexity and error propagation for straightforward low-risk profiles. <!-- [Evidence: trace P00363, iter 1] -->
